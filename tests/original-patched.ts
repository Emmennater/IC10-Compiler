...
export function compile(ast: SyntaxNode,
    ...
    switch (node.type) {
      ...
      case "BinaryOp": {
        ...
        if (op in ALU_OPCODES) {
          // PATCH 1: original folded "%" as division
          const value = op === "+" ? x + y : op === "-" ? x - y : op === "*" ? x * y : op === "/" ? x / y : ic10_mod(x, y);
  ...
  function processIf(node: SyntaxNode) {
    ...
    for (let i = 0; i < arms.length; i++) {
      ...
      // PATCH 2: scan the active emit buffer, not the main program buffer,
      // so ifs inside jal-lowered function bodies record their branches
      const branchIds = emitBuffer
  ...
  function processWhile(node: SyntaxNode) {
    ...
    const entryFolded = foldExpression(cond);
    ...
    // PATCH 5: fold the steady-state condition only after demoted
    // variables forgot their entry constants; the entry value must not
    // decide every iteration's branch when the body reassigns it
  ...
  function processRepeat(node: SyntaxNode) {
    ...
    // PATCH 3: read the active emit buffer, not the main program buffer
    const last = emitBuffer[emitBuffer.length - 1];
  ...
  function lowerFunction(fn: FnInfo, node: SyntaxNode) {
    ...
    // PATCH 7: a jal-lowered body must not see the caller's loops - a
    // break/continue inside it would jump across the function boundary to
    // a loop label chosen at whichever call site triggered lowering
    const savedLoopStack = loopStack.splice(0, loopStack.length);
    ...
    loopStack.push(...savedLoopStack); // PATCH 7
    loweringStack.pop();
    fn.lowered = buffer;
  }
  ...
  function allocateAndEmit(program: Inst[]): string {
    ...
    while (true) {
      ...
      // PATCH 6: an instruction may both use and define the victim
      // (`i += 1` lowers to `add home home 1`). The original's if/else-if
      // took only the "defines" branch, redirecting the destination but
      // never reloading the use - leaving the victim with uses and no
      // definition, which the next spill round then backed with a second,
      // never-written stack slot. Reload and store now share one scratch.
      ...
      for (const inst of program) {
        ...
        switch (inst.op) {
          ...
          case "branch":
          case "call":
            copy = { ...inst, args: inst.args.map(replace) };
            break;
          case "movev":
          case "storename":
          case "poke":
            copy = { ...inst, src: replace(inst.src) };
            break;
          default:
            copy = { ...inst };
            break;
...
// PATCH 4: typed map; only the final token of a jump/branch line is a label
// reference, so only that token is substituted (an alias/define sharing a
// label's name is left alone)
